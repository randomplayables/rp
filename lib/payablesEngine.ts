...
// Default weights for "other" contribution types (applied within the otherContributionsWeight bucket)
export const DEFAULT_OTHER_WEIGHTS = {
  gamePublicationWeight: 0.25,
  communityWeight: 0.15,
  codeWeight: 0.05,
  contentWeight: 0.05,
...
/**
 * Loads the weights for "Other Category" contributions from the database configuration.
 * Falls back to default weights if no configuration is found.
 */
export async function loadOtherWeights(): Promise<typeof DEFAULT_OTHER_WEIGHTS> {
    const config = await PayoutConfigModel.findOne().lean();
    if (config && config.weights) {
        // Ensure all keys exist, falling back to default for any missing ones
        return {
            gamePublicationWeight: config.weights.gamePublicationWeight ?? DEFAULT_OTHER_WEIGHTS.gamePublicationWeight,
            communityWeight: config.weights.communityWeight ?? DEFAULT_OTHER_WEIGHTS.communityWeight,
            codeWeight: config.weights.codeWeight ?? DEFAULT_OTHER_WEIGHTS.codeWeight,
            contentWeight: config.weights.contentWeight ?? DEFAULT_OTHER_WEIGHTS.contentWeight,
        };
    }
    return DEFAULT_OTHER_WEIGHTS;
}

/**
 * Calculates the weighted total points for the "Other Category" bucket.
 * @param metrics - An object containing the raw point values for each sub-category.
 * @param otherWeights - The weights to apply to each sub-category.
 * @returns The final weighted point total, rounded to 6 decimal places.
 */
export function getWeightedPoints(
    ...
    otherWeights: typeof DEFAULT_OTHER_WEIGHTS
): number {
    const weightedSum = (
        (metrics.gamePublicationPoints || 0) * otherWeights.gamePublicationWeight +
        (metrics.contentCreation || 0) * otherWeights.contentWeight +
        (metrics.codeContributions || 0) * otherWeights.codeWeight +
        (metrics.communityEngagement || 0) * otherWeights.communityWeight
    );
    // Round to 6 decimal places to avoid floating point inaccuracies
    return Math.round(weightedSum * 1e6) / 1e6;
}
