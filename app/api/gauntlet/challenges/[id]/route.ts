...
//         { error: "Challenge ID not found in URL" },
...
//       return NextResponse.json(
//         { error: "Opponent setup configuration is required." },
//         { status: 400, headers: corsHeaders }
//       );
//     }
...
//       return NextResponse.json(
//         { error: "This challenge is no longer open." },
//         { status: 400, headers: corsHeaders }
//       );
//     }
...
//       return NextResponse.json(
//         { error: "Insufficient points for this wager." },
//         { status: 400, headers: corsHeaders }
//       );
//     }
...
export async function POST(
  ...
  try {
    ...
    if (!userContribution || userBalance < opponentWager) {
      return NextResponse.json(
        { error: "Insufficient points in the required category for this wager." },
        { status: 400, headers: corsHeaders }
      );
    }
    
    const updatePayload: any = { $inc: { [`metrics.${balanceField}`]: -opponentWager }};
    if (challenge.wagerSubCategory) {
        updatePayload.$inc['metrics.totalPoints'] = -opponentWager;
    }
