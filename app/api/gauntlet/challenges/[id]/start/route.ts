...
export async function POST(
  ...
  try {
    ...
    if (!isPlayer) {
      return NextResponse.json({ error: "You are not a player in this challenge." }, { status: 403, headers: corsHeaders });
    }
    ...
    if (!updatedChallenge) {
      ...
      if (currentState && currentState.status === 'in_progress') {
        return NextResponse.json({ success: true, message: "Game already in progress." }, { headers: corsHeaders });
      }
      return NextResponse.json({ error: "Game could not be started. It might not be active anymore." }, { status: 409, headers: corsHeaders });
    }

    return NextResponse.json({ success: true, message: "Game started." }, { headers: corsHeaders });
  } catch (error: any) {
    console.error("Error starting gauntlet game:", error);
    return NextResponse.json({ error: "Failed to start game", details: error.message }, { status: 500, headers: getDynamicCorsHeaders(request) });
  }
}