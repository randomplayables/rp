...
/**
 * Helper function to call the AI chat API with a retry mechanism.
 * This is used specifically for the 'code' generation step of RPTS to improve reliability.
 */
...
): Promise<any> { // Returns a ChatCompletion object
  try {
    return await callOpenAIChat(model, messages);
  } catch (error) {
    if (retries > 0) {
      console.log(`AI call failed. Retrying in ${delay}ms... (${retries} retries left). Error: ${error instanceof Error ? error.message : String(error)}`);
      await new Promise(res => setTimeout(res, delay));
      // Recursively call with one less retry and doubled delay (exponential backoff)
      return callOpenAIChatWithRetry(model, messages, retries - 1, delay * 2);
    } else {
      console.error("AI call failed after multiple retries.");
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (useCodeReview) {
      ...
      let response;
      if (language === 'rpts' && generationStep === 'code') {
          console.log(`[GameLab API] Performing AI call for RPTS file generation with retry logic.`);
          response = await callOpenAIChatWithRetry(modelToUse, messagesToAI);
      } else {
          response = await callOpenAIChat(modelToUse, messagesToAI);
      }
