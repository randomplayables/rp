...
//     if (attachedAsset) {
//         const fileContent = await attachedAsset.text();
//         assetContext = `An asset has been attached with the following details:\n- Filename: ${attachedAsset.name}\n- Content:\n\`\`\`\n${fileContent}\n\`\`\``;
//     }
...
//     if (attachedAsset) {
//         assetPath = `/src/assets/${attachedAsset.name}`;
//         // For binary files like images, convert to a Base64 data URL for Sandpack
//         const fileBuffer = await attachedAsset.arrayBuffer();
//         const base64Content = Buffer.from(fileBuffer).toString('base64');
//         assetContent = `data:${attachedAsset.type};base64,${base64Content}`;

//         assetContext = `An asset has been attached. It will be placed at the following path: ${assetPath}. You MUST import it from this exact path in your generated code. For example: import myAsset from '${assetPath}';`;
//     }
...
//     // Add the asset to the files object if it exists
//     if (assetPath && assetContent && extractionResult.files) {
//         extractionResult.files[assetPath] = assetContent;
//     }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (attachedAsset) {
        ...
        const fileBuffer = await attachedAsset.arrayBuffer();
        const base64Content = Buffer.from(fileBuffer).toString('base64');
        assetContent = base64Content; // Store pure Base64, not a data URI
...
//         const fileBuffer = await attachedAsset.arrayBuffer();
//         const base64Content = Buffer.from(fileBuffer).toString('base64');
//         const dataUri = `data:${attachedAsset.type};base64,${base64Content}`;
        
//         // Sanitize asset name for use as a variable
//         const assetVarName = attachedAsset.name.replace(/[^a-zA-Z0-9_]/g, '_').replace(/^(\d)/, '_$1');

//         assetContext = `An asset named '${attachedAsset.name}' was attached. You MUST embed it directly into your component as a constant using its Data URI. Do NOT use a file import.
        
//         Example:
//         \`\`\`
//         const ${assetVarName} = '${dataUri}';

//         function App() {
//           return <img src={${assetVarName}} alt="Uploaded asset" />;
//         }
//         \`\`\`
//         `;
//     }
