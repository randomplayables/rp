  ...
  try {
    ...
    if (!modelResolution.canUseApi || modelResolution.limitReached) {
      return NextResponse.json({
        ...
        limitReached: true,
        dataContext: querySpecificDataContext // Return data context even if limit reached
    ...
    if (useCodeReview) {
      ...
      if (!initialCode.trim() && !revisedCode.trim()) {
         finalApiResponse = {
            message: `Chatbot1 (Model: ${chatbot1ModelToUse}) did not produce code. Initial: ${initialMessageText || reviewCycleOutputs.chatbot1InitialResponse.content}. Revision attempt also failed.`,
            code: "",
        };
      } else {
        finalApiResponse = {
    ...
    // Initial remaining requests (before this call's charge)
    ...
    // Attempt to return dataContext even in case of an error, if available
