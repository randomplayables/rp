...
import { useQuery } from '@tanstack/react-query';
import { Spinner } from '@/components/spinner';
import { IGauntletChallenge as GauntletChallengeType } from '@/models/Gauntlet'; 

interface IGauntletChallenge extends GauntletChallengeType {
  _id: string;
}

const otherCategorySubTypes: Record<string, string> = {
    gamePublicationPoints: 'Game Publications',
...
export default function GauntletPage() {
  ...
  const getSubCategoryLabel = (subCategoryKey?: string) => {
    if (!subCategoryKey) return 'Other Points';
    return otherCategorySubTypes[subCategoryKey] || 'Other Points';
  };
        ...
        {data?.challenges.map((challenge) => (
          <Link key={challenge._id as string} href={`/gauntlet/${challenge._id}`} className="block bg-white p-6 rounded-lg shadow hover:shadow-lg transition-shadow">
            ...
            <div className="mt-4 flex justify-between">
             ...
             {challenge.wagerSubCategory && (
                <div className="mt-3 pt-3 border-t border-gray-100">
                    <p className="text-xs text-gray-500">Points Category</p>
                    <p className="font-semibold text-sm text-emerald-700">{getSubCategoryLabel(challenge.wagerSubCategory)}</p>
                </div>
            )}
          </Link>
        ))}
