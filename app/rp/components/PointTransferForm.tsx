...
import { useMutation } from '@tanstack/react-query';
import { Spinner } from '@/components/spinner';
import { IUserContribution, ContributionMetrics } from '@/models/RandomPayables';
...
type PointType = 'totalPoints' | 'githubRepoPoints' | 'peerReviewPoints';
type OtherCategorySubType = keyof Pick<ContributionMetrics, 'gamePublicationPoints' | 'codeContributions' | 'contentCreation' | 'communityEngagement'>;
...
const otherCategorySubTypes: { value: OtherCategorySubType; label: string }[] = [
...
async function transferPointsAPI(payload: { recipientUsername: string; amount: number; memo?: string, pointType: PointType, otherCategorySubType?: OtherCategorySubType }) {
    const response = await fetch('/api/rp/transfer', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
...
export default function PointTransferForm({ userContribution, onTransferSuccess }: PointTransferFormProps) {
    ...
    const [memo, setMemo] = useState('');
    const [selectedPointType, setSelectedPointType] = useState<PointType>('totalPoints');
    const [selectedOtherSubType, setSelectedOtherSubType] = useState<OtherCategorySubType>(otherCategorySubTypes[0].value);

    const getBalance = (type: PointType, subType?: OtherCategorySubType) => {
        if (!userContribution?.metrics) return 0;
        if (type === 'totalPoints' && subType) {
            return userContribution.metrics[subType] || 0;
        }
        return userContribution.metrics[type] || 0;
    };
    
    const currentBalance = getBalance(selectedPointType, selectedOtherSubType);
    const currentBalanceFormatted = currentBalance.toFixed(2);
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        mutation.mutate({ 
            ...
            otherCategorySubType: selectedPointType === 'totalPoints' ? selectedOtherSubType : undefined
        });
    };
    ...
    return (
        <div className="bg-white shadow-sm rounded-lg p-6 border border-gray-200">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                {selectedPointType === 'totalPoints' && (
                    <div>
                        <label htmlFor="subCategory" className="block text-sm font-medium text-gray-700">From Bucket</label>
                        <select
                            id="subCategory"
                            value={selectedOtherSubType}
                            onChange={(e) => setSelectedOtherSubType(e.target.value as OtherCategorySubType)}
                            className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-emerald-500 focus:border-emerald-500"
                            disabled={mutation.isPending}
                        >
                            {otherCategorySubTypes.map(subType => (
                                <option key={subType.value} value={subType.value}>{subType.label} ({(userContribution?.metrics[subType.value] || 0).toFixed(2)})</option>
                            ))}
                        </select>
                    </div>
                )}
